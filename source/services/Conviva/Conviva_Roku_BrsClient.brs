...
' LivePass Version: 2.117.0.31695
...
function ConvivaLivePassInitWithSettings (apiKey as string, settings as object)
    ...
    self.createSession = function (screen as object , contentInfo as object, positionNotificationPeriod as float, clipInfo = invalid  as Dynamic, sessionCallback = invalid as Dynamic, callbackObj = invalid as Dynamic) as object
        self = m
        video = invalid
...
' Session class
'--------------
function cwsConvivaSession(cws as object, screen as object, contentInfo as object, notificationPeriod as float, clipInfo as Dynamic, notifyReadyCb as Dynamic, callbackObj as Dynamic, video as object) as object
    self = {}
    self.video = video
    self.screen = screen
...
'        end if 
	...
	if selectionAvailable = true
	    ...
	    if self.notifyReady <> invalid and self.isReady = false
                self.log("notifying self.cws.REASON_BACKEND_SELECTION_AVAILABLE xxxx")
            ...
            if self.notifyReady <> invalid and self.isReady = false
                self.log("notifying self.cws.REASON_BACKEND_SELECTION_UNAVAILABLE xxx")
