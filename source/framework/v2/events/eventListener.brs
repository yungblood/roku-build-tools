'IMPORTS=v2/base/globalObjectRegistry v2/events/eventPort v2/events/keyEventManager utilities/arrays
' ******************************************************
...
Sub EventListener_UnregisterObserverForAllEvents(observer As Object)
    ' TODO: Support custom event ports?
    For Each port In m.Ports
        'port = m.EventPort
        port.UnregisterObserverForAllEvents(observer)
    Next
End Sub
    ...
    While True
        ...
        If eventPort <> invalid Then
            ...
            If msg <> invalid Or eventPort.Timeout = -1 Or (eventPort.Timeout > 0 And eventPort.EventTimer.TotalMilliseconds() >= eventPort.Timeout) Then
                ...
                If msg = invalid Then
                    If KeyEventManager().IsKeyDown() Then
                        m.LastEventType = "roUniversalControlEvent"
                    Else
                ...
                If m.LastEventType = "roUniversalControlEvent" Then
                    ...
                    keyEvent = KeyEventManager().ProcessKeyEvent(msg)
                    If keyEvent <> invalid Then
                        result = eventPort.RaiseEvent("KeyEvent", keyEvent)
                    End If
                End If
