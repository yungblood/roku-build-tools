...
sub onItemSpacingChanged()
    m.list.itemSpacings = [m.top.itemSpacing]
end sub
...
sub initializeComponents()
    if m.list.getChildCount() = 0 and m.top.content <> invalid and not isNullOrEmpty(m.top.itemComponentName) then
        visibleItems = getVisibleItemCount()
        'for i = 0 to (visibleItems * 3) - 1
        for i = 0 to (visibleItems + 2) - 1
            component = m.list.createChild(m.top.itemComponentName)
